...
def lambda_handler(event, context):
    ...
    for loc in locations:
        df['location_'+loc.replace(" ","").strip()] = df['location'].str.contains(loc).astype(int)
    ...
    df[['smokes','sometimes', 'no', 'when drinking', 'yes', 'trying to quit']].head(5)
    ...
    df_for_prediction['age']= (np.abs(int(df_for_prediction['age'])-18))/(69-18)
    df_for_prediction['income']=(np.abs(int(df_for_prediction['income'])-0))/(1000000-0)
